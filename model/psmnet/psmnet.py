...
from model.psmnet.psmnet_submodule import *
...
class Adapter_PSMNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.adapter = Adapter()
        self.psmnet = PSMNet(maxdisp=192)

    def forward(self, img_L, img_R):
        img_L_transformed, img_R_transformed = self.adapter(img_L, img_R)
        output = self.psmnet(img_L, img_R, img_L_transformed, img_R_transformed)
        return output
