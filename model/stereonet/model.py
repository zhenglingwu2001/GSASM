...
class StereoNet(pl.LightningModule):
    ...
    def __init__(self, in_channels: int,
                 ...
                 scheduler_partial: Optional[Callable[[torch.optim.Optimizer], torch.optim.lr_scheduler._LRScheduler]] = None) -> None:
        super().__init__()
...
if __name__ == "__main__":
    model = StereoNet(in_channels=3, k_downsampling_layers=3, k_refinement_layers=3,
                      candidate_disparities=256, mask=True).cuda()
    sample = torch.rand(3, 6, 256, 512).cuda()
    out = model(sample)
